...
import csv
from kernels import kernel_set
...
def cnn(test_group: int):
    ...
    for i in range(10):
        ...
        model.add(layers.Dense(num_classes))  # 5 (white, blue, black, red, green)
        model.compile(
            optimizer="adam",
        ...
        with open("./res/test.csv", "a") as file:
...
def main():
    with open("./res/test.csv", "w+") as file:
        csv.writer(file).writerow(
            [
                "Test Group",
                "Run Number",
